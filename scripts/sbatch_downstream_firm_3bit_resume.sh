#!/bin/bash
# Downstream firm numbers (3-bit), resumable. Submit: sbatch scripts/sbatch_downstream_firm_3bit_resume.sh
...
#SBATCH --ntasks=1
#SBATCH --gres=gpu:4
#SBATCH --job-name=ds_firm_w3
#SBATCH --output=/capstor/scratch/cscs/fnoorala/ATLAS-PTQ/tacq_data/logs/downstream_firm_w3_%j.out
set -euo pipefail
...
PY
SEEDS="${SEEDS:-0 1 2}" WBITS=3 FORCE_RECOMPUTE="${FORCE_RECOMPUTE:-1}" \
