...
\subsection{Saliency-source ablation: what signal should select outliers?}
\label{sec:ablation}

Our central experiment holds \emph{everything} fixed---model
(Llama-3.1-8B-Instruct), FP16 budget (0.35\%, global top-$k$), contrastive
calibration pairs (128), GPTQ (true-sequential), and evaluation---and varies
only the saliency signal that ranks weights. Eight arms span the hypothesis
space from task-agnostic controls to the full conjunction
...
task loss and task circuit is what buys accuracy.
...
entirely with transcoder alignment (v1) ties \baseline{} (25.0 vs.\ 26.0),
consistent with the ablation: improvement requires the conjunction, not either
signal alone. At 3-bit, \baseline{} reaches 64.8\%, v1 ties (65.0\%), and
v2~mult reaches 66.2\% (seed 0)---again a compressed gap at the milder
bit-width.
